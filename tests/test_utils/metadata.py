...
def create_asset_files(asset_names, policy, request, metadata_dir):
    for asset_name in asset_names:
        asset_file = asset_filename(asset_name)
        sample_metadata_json = metadata_json(request, asset_file)
...
def metadata_json(request, asset_file):
